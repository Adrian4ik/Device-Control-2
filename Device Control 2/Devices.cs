...
{
    class Devices // файлы config.xml и optlist.xml добавляются в общую папку (устранить)
    {
        ...
        private string[] ReadDeviceList()
        {
            ...
            if (!File.Exists(path + "config.xml"))
                File.WriteAllText(path + "config.xml", "folder names:");
            else
            {
                string[] devices = File.ReadAllLines(path + "config.xml");
